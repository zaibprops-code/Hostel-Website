...
export function TrustBar() {
  return (
    <div className="border-y border-forest-900/8 bg-sand">
      <div data-reveal-group className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-8 gap-y-4 px-5 py-6 sm:px-8">
        {promises.map((p) => (
          <div
            key={p.label}
            data-reveal
            className="group flex items-center gap-2.5 text-sm font-medium text-forest-800"
          >
            <Icon
              name={p.icon}
              size={20}
              className="text-brass-500 transition-transform duration-300 group-hover:scale-110"
            />
            {p.label}
          </div>
        ))}
