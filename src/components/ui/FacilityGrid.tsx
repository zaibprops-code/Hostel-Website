...
export function FacilityGrid({
  ...
  if (variant === "compact") {
    return (
      <ul data-reveal-group className={cn("grid grid-cols-2 gap-3", cols)}>
        {facilities.map((f) => (
          <li
            key={f.id}
            data-reveal
            className="flex items-center gap-3 rounded-xl border border-forest-900/8 bg-white/60 px-4 py-3 transition-colors duration-300 hover:border-brass-200 hover:bg-white"
          >
  ...
  return (
    <ul data-reveal-group className={cn("grid gap-5", cols)}>
      {facilities.map((f) => (
        <li
          key={f.id}
          data-reveal
